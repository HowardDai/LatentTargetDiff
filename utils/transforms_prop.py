...
class FeaturizeLigandAtom(object):
    ...
    def __init__(self):
        super().__init__()
        self.atomic_numbers = torch.LongTensor([1,  5,  6,  7,  8,  9, 13, 14, 15, 16, 17, 33, 35, 53, 80, 83])  #  HD CHANGED: GeoLDM inputs
...
class EncodeLigandAtom(object): #HD class adding GeoLDM
    def __init__(self):
        super().__init__()
    ...
    @property
    def feature_dim(self):
        return self.VAE.latent_node_nf
    
    def __call__(self, data: ProteinLigandData):
        z_x_mu, sigma_0_x, z_h_mu, sigma_0_h = self.VAE.encode(data.ligand_pos, data.ligand_atom_feature_full) 
        data.ligand_pos = torch.squeeze(z_x_mu)
        data.ligand_atom_feature_full = torch.squeeze(z_h_mu)
        return data
