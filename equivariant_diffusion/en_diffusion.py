...
class EnHierarchicalVAE(torch.nn.Module):
    ...
    def encode(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        # h_one_hot = torch.zeros(x.size(dim=0), self.in_node_nf)
        # for idx in range(0, x.size(dim=0)):
        #     h[idx, h[idx]] = 1
        h = torch.unsqueeze(h, 0) # HD - now encoding one at a time
        x = torch.unsqueeze(x, 0)
        # print(x.size())
        # print(h_one_hot.size())
    ...
    def decode(self, z_xh, node_mask=None, edge_mask=None, context=None):
        """Computes p(x|z)."""
        # node_mask = torch.zeros(z_xh.size(dim=0), dtype=torch.int64)
        # edge_mask = torch.zeros([z_xh.size(dim=0), z_xh.size(dim=0)], dtype=torch.int64)
        z_xh = torch.unsqueeze(z_xh, 0)
        # Decoder output (reconstruction).
        ...
        h_int = xh[:, :, -1:] if self.include_charges else torch.zeros(0).to(xh) # HD: check the alignment of this? 
