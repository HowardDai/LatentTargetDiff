...
class ScorePosNet3D(nn.Module):
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
        ...
        init_ligand_h = self.ligand_atom_emb(input_ligand_feat) #use encoder here?? 
        ...
        final_ligand_v = self.v_inference(final_ligand_h) #use decoder here 
    ...
    def q_v_pred_one_timestep(self, log_vt_1, t, batch):
        ...
        log_probs = log_add_exp(
            ...
            log_1_min_alpha_t - np.log(16) #self.num_classes
        )
        return log_probs
    ...
    def q_v_pred(self, log_v0, t, batch):
        ...
        log_probs = log_add_exp(
            ...
            log_1_min_cumprod_alpha - np.log(16)# self.num_classes
        )
        return log_probs
    ...
    def q_v_sample(self, log_v0, t, batch):
        ...
        log_sample = index_to_log_onehot(sample_index, 16 )#self.num_classes)
    ...
    def get_diffusion_loss(
            self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, autoencoder, node_mask, edge_mask, time_step=None, 
    ):
        ...
        log_ligand_v0 = index_to_log_onehot(ligand_v, num_classes= 16) # could generalize this
        
        encoded_ligand_pos, _, encoded_ligand_v, _ = autoencoder.encode(ligand_pos, log_ligand_v0, node_mask, edge_mask) 
        encoded_ligand_pos = encoded_ligand_pos.squeeze()
        encoded_ligand_v = encoded_ligand_v.squeeze()
        pos_noise = torch.zeros_like(encoded_ligand_pos)
        pos_noise.normal_()
        ligand_pos_perturbed = a_pos.sqrt() * encoded_ligand_pos + (1.0 - a_pos).sqrt() * pos_noise  # pos_noise * std
        ...
        preds = self(
            ...
            init_ligand_v=encoded_ligand_v,
            batch_ligand=batch_ligand,
            time_step=time_step
        )

        pred_ligand_pos, pred_ligand_v = preds['pred_ligand_pos'], preds['pred_ligand_v']
        # print(pred_ligand_v.size())
        pred_z_xh = torch.cat([pred_ligand_pos, pred_ligand_v], dim=1) # translate back into indexed format
        # print(pred_z_xh.size())
        # print(node_mask.size())
        pred_ligand_pos, h = autoencoder.decode(pred_z_xh, node_mask, edge_mask)
        pred_ligand_v = h['categorical']
        ...
        if self.model_mean_type == 'C0':
            target, pred = encoded_ligand_pos, pred_ligand_pos
        ...
        # self.VAE = getAutoEncoder.main() 
        
        
        # z_xh = torch.cat([ligand_pos, ligand_v], dim=1)
        # ligand_pos, ligand_v = self.VAE.decode(pred_z_xh)
        # atom type loss
    ...
    def likelihood_estimation(
        ...
        if (time_step == self.num_timesteps).all():
            ...
            log_ligand_v0 = batch_ligand # index_to_log_onehot(batch_ligand, self.num_classes)
        ...
        # Vt = a * V0 + (1-a) / K
        log_ligand_v0 = ligand_v # index_to_log_onehot(ligand_v, self.num_classes)
        ligand_v_perturbed, log_ligand_vt = self.q_v_sample(log_ligand_v0, time_step, batch_ligand)
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, init_ligand_v, batch_ligand, autoencoder, node_mask, edge_mask,
        ...
        log_ligand_v0 = index_to_log_onehot(ligand_v, num_classes= 16)
        print(ligand_pos.size())
        print(log_ligand_v0.size())
        print(node_mask.size())
        encoded_ligand_pos, _, encoded_ligand_v, _ = autoencoder.encode(ligand_pos, log_ligand_v0, node_mask, edge_mask) 
        encoded_ligand_pos = encoded_ligand_pos.squeeze()
        encoded_ligand_v = encoded_ligand_v.squeeze()
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            preds = self(
                ...
                init_ligand_pos=encoded_ligand_pos,
                init_ligand_v=encoded_ligand_v,
                batch_ligand=batch_ligand,
                time_step=t
            )
            ...
            pred_z_xh = torch.cat([pred_ligand_pos, pred_ligand_v], dim=1) 
          
            pred_ligand_pos, h = autoencoder.decode(pred_z_xh, node_mask, edge_mask)
            pred_ligand_v = h['categorical']
            
            atom_type_mask = torch.tensor([0, float('-inf'), 0, 0, 0, 0, float('-inf'), float('-inf'), 0, 0, 0, float('-inf'), float('-inf'), float('-inf'), float('-inf'), float('-inf')]).to(protein_pos.device)
            pred_ligand_v = pred_ligand_v + atom_type_mask
            
            # Compute posterior mean and variance
            if self.model_mean_type == 'noise':
                pred_pos_noise = ppred_ligand_pos - ligand_pos
            ...
            if not pos_only:
                ...
                log_ligand_v = log_ligand_v0 + atom_type_mask #index_to_log_onehot(ligand_v, 16)
                log_model_prob = self.q_v_posterior(log_ligand_v_recon, log_ligand_v, t, batch_ligand)
                log_model_prob = log_model_prob + atom_type_mask
